...
class SAC(nn.Module):
    ...
    def actor_update(self, states):
        ...
        return loss,now_action_log_prob, q.mean()
    ...
    def train_network(self, writer, n_epi, states, actions, rewards, next_states, done_masks):
        ...
        ### actor update
        actor_loss,prob, qforactor = self.actor_update(states)
        ...
        if writer != None:
            ...
            writer.add_scalar("loss/q_for_actor", qforactor, n_epi)
    ...
    def save(self, filename):
        torch.save(self.q_1.state_dict(), filename + "_q_1")
        torch.save(self.q_2.state_dict(), filename + "_q_2")
        torch.save(self.target_q_1.state_dict(), filename + "_target_q_1")
        torch.save(self.target_q_2.state_dict(), filename + "_target_q_2")
        torch.save(self.q_1_optimizer.state_dict(), filename + "_q_1_optimizer")
        torch.save(self.q_2_optimizer.state_dict(), filename + "_q_2_optimizer")

        torch.save(self.actor.state_dict(), filename + "_actor")
        torch.save(self.actor_optimizer.state_dict(), filename + "_actor_optimizer")

        torch.save(self.alpha.data, filename + "_alpha")
        torch.save(self.alpha_optimizer.state_dict(), filename + "_.alpha_optimizer")

    def load(self, filename):
        self.q_1.load_state_dict(torch.load(filename + "_q_1"))
        self.q_2.load_state_dict(torch.load(filename + "_q_2"))
        self.target_q_1.load_state_dict(torch.load(filename + "_target_q_1"))
        self.target_q_2.load_state_dict(torch.load(filename + "_target_q_2"))
        self.q_1_optimizer.load_state_dict(torch.load(filename + "_q_1_optimizer"))
        self.q_2_optimizer.load_state_dict(torch.load(filename + "_q_2_optimizer"))

        self.actor.load_state_dict(torch.load(filename + "_actor"))
        self.actor_optimizer.load_state_dict(torch.load(filename + "_actor_optimizer"))
        self.alpha = torch.nn.Parameter(torch.load(filename + '_alpha'))
        # self.alpha.load_state_dict(torch.load(filename + "_alpha"))
        self.alpha_optimizer.load_state_dict(torch.load(filename + "_.alpha_optimizer"))